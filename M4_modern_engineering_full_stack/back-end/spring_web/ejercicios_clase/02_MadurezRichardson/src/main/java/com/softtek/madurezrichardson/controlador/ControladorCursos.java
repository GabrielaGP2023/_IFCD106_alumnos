package com.softtek.madurezrichardson.controlador;
import static org.springframework.hateoas.server.mvc.WebMvcLinkBuilder.linkTo;
import static org.springframework.hateoas.server.mvc.WebMvcLinkBuilder.methodOn;
import com.softtek.madurezrichardson.excepciones.ExcepcionNoEncontradoModelo;
import com.softtek.madurezrichardson.modelo.Curso;
import com.softtek.madurezrichardson.servicio.ICursoServicio;
...
//    }
    @PostMapping
    public ResponseEntity<Void> crear(@RequestBody Curso curso) throws Exception{
        Curso obj = servicio.crear(curso);

        URI location = ServletUriComponentsBuilder.fromCurrentRequest()
                .path("/{id}").buildAndExpand(obj.getIdCurso()).toUri();
